...
- **`gamelog_join_leave`（全局表）是"谁在哪个实例进出的"**：`created_at, type, location, user_id, display_name, time`，`type` 只有 `OnPlayerJoined` / `OnPlayerLeft`，成对出现（实测 5164 / 5158）。
  ...
  - **成对记录会自己抖断**：同一对（好友 × 实例）里，实测 107 个"再次进入"有 42 个间隔不到 2 分钟（最短 12 秒），那是掉线重连或玩家列表被重写，不是真的离开又回来。`buildMeetings` 因此把**同一个人同一个房间**间隔 ≤ `DEFAULT_MEETING_MERGE_MS`（2 分钟）的两段并成一次见面。真实数据上 413 次 → 371 次，场数不变（187）。合并后的一次会带 `bounces` 计数。
